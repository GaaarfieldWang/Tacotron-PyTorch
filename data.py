...
class KSSDatasets(Dataset):
    ...
    def __init__(self, txt_file, root_dir):
        f = open(txt_file)
        self.txt_file = f.readlines()
        self.txt_file = [ele.split('|') for ele in self.txt_file]
        self.root_dir = root_dir
        
    def load_wav(self, filename):
        y, sr = librosa.load(filename, sr=hp.sample_rate)
        return librosa.effects.trim(y, top_db = 40)
    
    def __len__(self):
        return len(self.txt_file)
    
    def __getitem__(self, idx):
        wav_name = os.path.join(self.root_dir, self.txt_file[idx][0])
        text = self.txt_file[idx][2]
        text = np.asarray(text_to_sequence(text), dtype=np.int32)
        wav = np.asarray(self.load_wav(wav_name)[0], dtype=np.float32)
        sample = {'text': text, 'wav': wav}
...
def get_dataset():
    ...
    if hp.dataset == 'LJSpeech':
        ...
        return KSSDatasets(os.path.join(hp.data_path, 'transcript.v.1.3.txt'), os.path.join(hp.data_path, 'kss'))
